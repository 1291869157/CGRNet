...
if __name__ == "__main__":
    image = cv2.imread("2092.jpg") / 255
    out = cv2.imread("2092.png")[:,:,0] / 255
    out = out.reshape(out.shape[0],out.shape[1],1)
    image = torch.from_numpy(image).unsqueeze(0).permute(0,3,1,2)
    out = torch.from_numpy(out).unsqueeze(0).permute(0,3,1,2)
    image /= 0.1
    sample = {"rgb": image}
    loss_lsc_kernels_desc_defaults = [{"weight": 1, "xy": 6, "rgb": 0.1}]
    loss_lsc_radius = 5
    loss_lsc = LocalSaliencyCoherence()
    loss = loss_lsc(out, loss_lsc_kernels_desc_defaults, loss_lsc_radius, sample, image.shape[2], image.shape[3])['loss']
